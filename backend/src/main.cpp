    ...
    try{
        ...
        while(true){
            ...
            if(aircraft != nullptr){
                ...
                if(aircraft->vertical_rate.has_value()){
                    json["vertical_rate"] =
                        aircraft->vertical_rate.value();
                }

                if(aircraft->squawk.has_value()){
                    json["squawk"] =
                        aircraft->squawk.value();
                }

                if(aircraft->alert.has_value()){
                    json["alert"] =
                        aircraft->alert.value();
                }

                if(aircraft->emergency.has_value()){
                    json["emergency"] =
                        aircraft->emergency.value();
                }
                ...
                if(aircraft->is_on_ground.has_value()){
                    json["is_on_ground"] =
                        aircraft->is_on_ground.value();
                }

                json["last_seen_seconds"] =
                    aircraft->seconds_since_last_seen().count();
        ...
        */
        ...
        while(true){
            ...
            if(aircraft.altitude.value() >= 40000){
                altitudeChange =
                    -1000;
            }

            if(aircraft.altitude.value() <= 10000){
                altitudeChange =
                    1000;
            }

            if(aircraft.ground_speed.value() >= 900){
                speedChange =
                    -5;
            }

            if(aircraft.ground_speed.value() <= 450){
                speedChange =
                    5;
            }
